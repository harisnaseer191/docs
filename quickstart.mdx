...
When using API keys that start with `sk_test_`, you are in **test mode**:
...
When using API keys that start with `sk_live_`:
...
- ✅ Obtain production API key (`sk_live_`)
...
<Warning>
  **Critical Reminders for Production:**
  ...
  - Use actual customer phone numbers and CNICs
