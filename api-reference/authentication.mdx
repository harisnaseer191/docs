...
```bash cURL
curl --request GET \
  --url https://api.waypay.com/Gateway/v1/Transaction/by-ref/TXN123456 \
...
const response = await fetch(
  'https://api.waypay.com/Gateway/v1/Transaction/by-ref/TXN123456',
  {
    method: 'GET',
    headers: {
...
curl_setopt_array($curl, [
  CURLOPT_URL => "https://api.waypay.com/Gateway/v1/Transaction/by-ref/TXN123456",
...
```bash cURL
curl --request GET \
  --url https://api.waypay.com/Gateway/v1/Settlements/schedule \
...
const testAuth = async () => {
  try {
    const response = await fetch(
      'https://api.waypay.com/Gateway/v1/Settlements/schedule',
      {
        headers: {
          'Authorization': `Bearer ${process.env.WAYPAY_API_KEY}`
        }
